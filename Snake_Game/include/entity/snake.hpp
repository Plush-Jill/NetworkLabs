...
private:
    int m_id = 0;
    std::shared_ptr<std::deque<CoordPoint>> m_segments = {};
...
public:
    ...
    [[nodiscard]] std::shared_ptr<std::deque<CoordPoint>> get_segments() const;
    ...
    void set_id(int id);
    void set_segments(const std::shared_ptr<std::deque<CoordPoint>> segments);
